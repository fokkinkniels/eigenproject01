...
class Register extends Controller{
    ...
    public function RegisterFunction($post_data){
        ...
        if(!empty(Database::query("SELECT * FROM user WHERE name=:name", array(":name"=>$post_data['username']))[0])){
            $this->addError('username', 'username is already taken');   
        }
        else if(!empty(Database::query("SELECT * FROM user WHERE email=:email", array(":email"=>$post_data['email']))[0])){
        ...
        if(!empty($errors)){
            ...
            Database::query("INSERT INTO user(name, email, password) VALUES(?, ?, ?)",
            ...
            Database::query('INSERT INTO profileimg(userid, status) VALUES(?, ?)',
            array($user['ID'] , 1));
